...
export const loadModels = async () => {
  try {
    ...
    if (!window.ort) {
      throw new Error(
        "ONNX Runtime library not loaded. Check index.html script tag.",
      );
    }
    ...
    if (!onnxSession) {
      onnxSession = await ort.InferenceSession.create(
        "/models/mobilefacenet_insightface.onnx",
      );
...
export const calculatePose = (landmarks) => {
  ...
  const pitch =
    ((nose.y - eyesCenter.y) / Math.abs(chin.y - eyesCenter.y)) * 90;
...
export const alignFace = (imageElement, landmarks, outputSize = 112) => {
  ...
  const rightEye = {
    x:
      ((landmarks[rightEyeIdx[0]].x + landmarks[rightEyeIdx[1]].x) / 2) * width,
    y:
      ((landmarks[rightEyeIdx[0]].y + landmarks[rightEyeIdx[1]].y) / 2) *
      height,
  };
  ...
  ctx.drawImage(imageElement, -eyeCenter.x, -eyeCenter.y, width, height);
...
export const getFaceEmbedding = async (imageElement, landmarks = null) => {
  ...
  for (let i = 0; i < 112; i++) {
    for (let j = 0; j < 112; j++) {
      ...
      // Red channel
      inputTensor[0 * 112 * 112 + i * 112 + j] =
        (pixels[pixelIndex] - 127.5) / 128.0;
      // Green channel
      inputTensor[1 * 112 * 112 + i * 112 + j] =
        (pixels[pixelIndex + 1] - 127.5) / 128.0;
      // Blue channel
      inputTensor[2 * 112 * 112 + i * 112 + j] =
        (pixels[pixelIndex + 2] - 127.5) / 128.0;
    }
  }
  ...
  try {
    ...
    // L2-normalize the embedding for proper cosine similarity
    const embArray = Array.from(rawEmbedding);
    const norm = Math.sqrt(embArray.reduce((sum, v) => sum + v * v, 0));
    if (norm > 0) {
      return embArray.map((v) => v / norm);
    }
    return embArray;
  } catch (error) {
